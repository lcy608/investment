...
def main():
    ...
    if view_mode == "현재 매물 추이":
        ...
        # 1. Aggregate Volume by Month
        v_df = filtered_df.copy()
        v_df['Month'] = v_df['Date'].dt.to_period('M').astype(str)
        volume_df = v_df.sort_values('Date').groupby('Month').size().reset_index(name='Volume')
        
        # 2. Create Figure with secondary Y-axis
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        
        # 3. Add Volume Bar Chart (Secondary Y-axis)
        fig.add_trace(
            go.Bar(x=volume_df['Month'], y=volume_df['Volume'], 
                   name='거래량 (월별)', marker_color='rgba(100, 149, 237, 0.3)',
                   hovertemplate='월: %{x}<br>거래량: %{y}건'),
            secondary_y=True
        )
        
        # 4. Add Price Scatter Plot (Primary Y-axis)
        filtered_df['Label'] = filtered_df['아파트명'] + " (" + filtered_df['평형_display'] + " - " + filtered_df['거래구분'] + ")"
        
        scatter_fig = px.scatter(filtered_df, x='Date', y='정렬용가격', color='Label', 
                                 size='정렬용가격', hover_data=['가격', '층', '거래구분'] if '거래구분' in filtered_df.columns else ['가격', '층'])
        
        for trace in scatter_fig.data:
            trace.update(marker=dict(opacity=0.8))
            fig.add_trace(trace, secondary_y=False)
            
        # 5. Update Layout
        fig.update_layout(
            height=600, 
            title_text="최근 5년 실거래가 및 거래량 통합 추이",
            showlegend=True,
        ...
        fig.update_yaxes(title_text="거래가 (만원)", secondary_y=False)
        fig.update_yaxes(title_text="거래량 (건수)", secondary_y=True, showgrid=False)
