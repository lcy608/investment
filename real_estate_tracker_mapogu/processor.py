...
def process_data(df):
    ...
    final_columns = ['articleNo', 'articleName', 'tradeTypeName', 'floorInfo', '가격', 'sort_price',
                     'area1', 'area2', 'area3', 'area4', 'direction', 'totalHouseholdCount', 'useApproveYmd']
    ...
    # Rename columns flexibly based on what was included
    column_names = {
        "articleNo": "매물번호",
        "articleName": "아파트명",
        "tradeTypeName": "거래타입",
    ...
    df_min_prices.columns = [column_names.get(c, c) for c in df_min_prices.columns]
